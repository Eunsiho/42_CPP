...
bool Conversion::checkFloat()
{
	...
	if (((*pos && !std::strcmp(pos, "f")) && (_str.find('.') != std::string::npos && _str.find('f', _str.length() - 1) == _str.length() - 1)) || !_str.compare("inff") || (!_str.compare("-inff") || !_str.compare("+inff") || !_str.compare("nanf")))
	{
		_dStr = tmp;
...
void Conversion::toInt()
{
	try
	{
		...
		if (!_str.compare("nan") || !_str.compare("nanf") || !_str.compare("inf") || !_str.compare("inff") || !_str.compare("+inf") || !_str.compare("+inff") || !_str.compare("-inf") || !_str.compare("-inff") || _dStr < INT_MIN || _dStr > INT_MAX)
			throw ImpossibleException();
		if (atol(_str.c_str()) > INT_MAX || atol(_str.c_str()) < INT_MIN)
			throw ImpossibleException();
